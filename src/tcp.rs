pub mod packet {
    ...
    pub fn send_tcp_packets(destination_ip: Ipv4Addr, interface: String, count: u64) {
        ...
        for interface in interfaces.iter() {
            let iface_ip = interface.ips.iter().next().map(|x| match x.ip() {
                ...
                _ => panic!(
                    "ERR - Interface IP is IPv6 (or unknown) which is not currently supported"
                ),
            });
        ...
        for i in 0..count {
            if &i % 10000 == 0 {
                println!("Sent {:?} packets to {}", &i, &destination_ip);
            }
